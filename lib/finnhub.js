...
export async function getQuote(rawSymbol) {
  ...
  if (isIndianSymbol(symbol)) {
    // Yahoo first: it's free/keyless with no fixed daily quota, and the
    // live poll interval hits this for every Indian holding on every tick
    // — Twelve Data's free tier (800 credits/day) gets exhausted in
    // minutes if it's queried that often, which silently turns every
    // Indian holding's price back into "unavailable" for the rest of the
    // day. Twelve Data is kept only as the fallback for when Yahoo has no
  ...
  const { ok, data } = await serverFetch(url('/quote', { symbol }), { ttl: 2500 });
